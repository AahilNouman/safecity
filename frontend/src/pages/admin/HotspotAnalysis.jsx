          ...
          >
            ...
            {hotspots.map((hotspot) => (
              <Circle
                ...
                radius={hotspot.radius * 1000} // km to meters
                pathOptions={{
                  color: '#E11D48',
                  fillColor: '#E11D48',
                  fillOpacity: selectedHotspot?.clusterId === hotspot.clusterId ? 0.4 : 0.2,
                  weight: 2,
                }}
                eventHandlers={{
              ...
              >
                <Popup>
                  <strong>Cluster {hotspot.clusterId}</strong><br/>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', overflowY: 'auto' }}>
          ...
          {hotspots.map(hotspot => (
            <div 
              ...
              style={{ 
                cursor: 'pointer',
                borderColor: selectedHotspot?.clusterId === hotspot.clusterId ? 'var(--rose)' : 'var(--border)',
                borderWidth: selectedHotspot?.clusterId === hotspot.clusterId ? '2px' : '1px'
              }}
              onClick={() => setSelectedHotspot(hotspot)}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                <h4 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <MapPin size={18} color="var(--rose)" /> Cluster {hotspot.clusterId}
                </h4>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  ...
                  <strong>{hotspot.primaryCategory}</strong>
                </div>
                <div>
                  <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', margin: '0 0 0.25rem 0' }}>Incidents</p>
                  <strong>{hotspot.incidentCount} reports</strong>
                </div>
                <div style={{ gridColumn: 'span 2' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <div style={{ flex: 1, height: '8px', background: 'var(--bg-page)', borderRadius: '4px', overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${(hotspot.averageSeverity / 10) * 100}%`, background: hotspot.averageSeverity > 7 ? 'var(--rose)' : 'var(--amber)' }} />
                    </div>
                    <span style={{ fontWeight: 'bold' }}>{hotspot.averageSeverity.toFixed(1)}</span>
                  </div>
                </div>
              </div>
            </div>
